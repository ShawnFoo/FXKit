...
@protocol FXInnerOverlayCacheKeyValue <NSObject>
- (NSString *)overlayCacheKeyValue;
@end

#pragma mark - FXInnerOverlayBorder
@interface FXInnerOverlayBorder : NSObject <FXInnerOverlayCacheKeyValue>
...
@implementation FXInnerOverlayBorder

+ (instancetype)borderWithWidth:(CGFloat)width andColor:(UIColor *)color {
	FXInnerOverlayBorder *border = [[FXInnerOverlayBorder alloc] init];
	border.width = width;
	border.color = color;
	return border;
}
...
#pragma mark - FXInnerOverlayModel
@interface FXInnerOverlayModel : NSObject <FXInnerOverlayCacheKeyValue>

@property (nonatomic, assign) CGFloat radius;
@property (nonatomic, assign) CGSize size;
@property (nonatomic, strong) UIColor *bgColor;
@property (nonatomic, assign) UIRectCorner corners;
@property (nonatomic, strong, nullable) FXInnerOverlayBorder *border;

@end

@implementation FXInnerOverlayModel

+ (instancetype)modelWithRadius:(CGFloat)radius
						   size:(CGSize)size
					 background:(UIColor *)bgColor
						corners:(UIRectCorner)corners
						 border:(nullable FXInnerOverlayBorder *)border {
	FXInnerOverlayModel *model = [[FXInnerOverlayModel alloc] init];
	model.radius = radius;
	model.size = size;
...
- (void)fx_cornerRadius:(CGFloat)radius
	...
	FXInnerOverlayBorder *border = nil;
	if (borderWidth > 0
		&& borderColor
		&& borderWidth < radius) {// 内边框半径不得大于圆角半径
		border = [FXInnerOverlayBorder borderWithWidth:borderWidth andColor:borderColor];
	}
	FXInnerOverlayModel *model = [FXInnerOverlayModel modelWithRadius:radius
																 size:size
														   background:bgColor
															  corners:corners
															   border:border];
	...
	self.fx_overlayImageView.frame = CGRectMake(0, 0, size.width, size.height);
...
- (UIImage *)fx_overlayImageForModel:(FXInnerOverlayModel *)model {
...
- (UIImage *)fx_createImageForModel:(FXInnerOverlayModel *)model {
...
- (UIBezierPath *)fx_overlayPathForModel:(FXInnerOverlayModel *)model withRect:(CGRect)rect {
...
- (UIBezierPath *)fx_borderPathForModel:(FXInnerOverlayModel *)model {
